...
- **응답 record 컴포넌트는 기본이 "항상 온다"이다.** `RecordRequiredModelConverter`가 모든 컴포넌트를
  ...
  깨진다. 붙이면 코틀린에서 `String?`이 되어 앱이 분기하게 된다. **한 응답이 갈래마다 다른 필드를
  채운다면 그 필드는 전부 `@Nullable`이다** — `KakaoLoginResponse`는 기존 회원에겐 토큰을, 신규에겐
  가입 토큰을 채우므로 어느 쪽이든 나머지가 null로 나간다.
