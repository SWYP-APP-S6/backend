...
- **`repository`만 JPA 영속성 API**(`JpaRepository`/`EntityManager`)에 접근한다. controller·service·
  function은 직접 쓰지 않는다.
- **repository를 부르는 건 같은 feature의 `function`뿐이다** — service가 직접 주입하지 않는다.
...
- **다른 feature의 repository를 직접 호출하지 않는다** — 상대 feature의 `function`을 경유.
